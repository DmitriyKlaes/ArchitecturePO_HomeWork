...
public class Editor3D implements UILayer{
    ...
    public void addNewModel() {
        ...
        Model3D model3D = this.databaseAccess.getRandomModel();
        businessLogicalLayer.addNewModel(model3D);

        System.out.printf("Модель %d успешно добавлена%n", model3D.getId());
    }
    ...
    public void deleteModel(int modelId) {
        ArrayList<Model3D> models = (ArrayList<Model3D>)businessLogicalLayer.getAllModels();
        if (modelId < 0) {
            throw new RuntimeException(String.format("Модели %d не существует", modelId));
        }

        Model3D model3D = models.stream().filter(model -> model.getId() == modelId)
                                            .findFirst()
                                                .orElseThrow(RuntimeException::new);

        businessLogicalLayer.removeModel(model3D);
